...
class Language extends MY_Controller {
        ...
        if($_POST){
            ...
            if ($check == 1) {
                ...
                $data=array(
                    ...
                    'arabic' => $this->input->post('arabic', true)
                );
                $data = $this->security->xss_clean($data);
                $this->admin_model->insert($data, 'lang_values');
                $this->session->set_flashdata('msg', trans('msg-inserted')); 
                redirect(base_url('admin/language/values/'.$this->input->post('lang', true)));
            }
        }
    }
    ...
    {   
        ...
        $data['page_title'] = 'language';  
        $data['value'] = $slug;  
        $data['language'] = $this->admin_model->select('lang_values');
    ...
    {   
        $data = array();
        $languages = $this->admin_model->select('lang_values');
    ...
    public function active($id) 
    {
        ...
        $this->session->set_flashdata('msg', trans('msg-activate')); 
        redirect(base_url('admin/language'));
    }
    ...
    public function deactive($id) 
    {
        ...
        if ($language->name != 'english') {
            $this->admin_model->update($data, $id,'language');
            $this->session->set_flashdata('msg', trans('msg-deactivate')); 
        }
        redirect(base_url('admin/language'));
    }
